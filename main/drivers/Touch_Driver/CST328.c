...
/* Shared new-driver I2C master bus for the touch controllers (port I2C_Touch_MASTER_NUM) */
...
/**
 * @brief Lazily create (once) the shared new-driver I2C master bus on the touch port.
 */
esp_err_t Touch_Bus_Get(i2c_master_bus_handle_t *out_bus)
{
    if (out_bus == NULL) {
    ...
    if (s_touch_bus == NULL) {
        ...
        ESP_RETURN_ON_ERROR(i2c_new_master_bus(&bus_config, &s_touch_bus), TAG, "create touch I2C bus failed");
    }
...
esp_err_t TOUCH_Init(esp_lcd_touch_handle_t *out_tp)
{
    esp_err_t ret;
    ...
    *out_tp = NULL;
    ...
    ret = i2c_master_probe(bus_handle, ESP_LCD_TOUCH_IO_I2C_CST328_ADDRESS, pdMS_TO_TICKS(100));
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "No device found at I2C address 0x%02X", ESP_LCD_TOUCH_IO_I2C_CST328_ADDRESS);
        return ESP_ERR_NOT_FOUND;
    }

    ESP_LOGI(TAG, "Touch device found at I2C address 0x%02X", ESP_LCD_TOUCH_IO_I2C_CST328_ADDRESS);
    ...
    ESP_RETURN_ON_ERROR(
        esp_lcd_touch_new_i2c_cst328(tp_io_handle, &tp_cfg, &local_tp),
        TAG,
